  ...
  try {
    ...
    if (!subject_id) {
      ...
      periodQuery = `
        ...
        ${condition}
        GROUP BY a.subject_id, sub.name, s.section, a.period
        ORDER BY sub.name, s.section, a.period
      `;

      periods = await db.query(periodQuery, params);
    } else {
      // 🔥 SUBJECT SELECTED → SECTION WISE
      periodQuery = `
        SELECT
          s.section,
          a.period,
        ...
        JOIN students s ON a.student_id = s.id
        ${condition}
        GROUP BY s.section, a.period
        ORDER BY s.section, a.period
      `;
