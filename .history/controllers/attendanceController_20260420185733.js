...
exports.markAttendance = async (req, res) => {
  ...
  try {
    // ✅ FIX 1: REMOVE section mandatory check
    if (!date || !attendance?.length) {
      return res.status(400).json({ message: "Missing required fields" });
    }

    // ✅ Normalize section (can be null)
    const normalizedSection = section && section !== "" ? section : null;
      ...
      for (let rec of attendance) {
        await db.query(
          ...
          [
            ...
            p,
            rec.status,
            normalizedSection, // ✅ FIX 2: use normalized value
          ],
        );
      }
    }
