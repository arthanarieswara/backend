...
exports.markAttendance = async (req, res) => {
  const { class_id, subject_id, date, attendance } = req.body;
  ...
  try {
    ...
    /* ✅ VALIDATION */
    if (!subject_id || !date || !attendance?.length) {
      return res.status(400).json({
        message: "Missing required fields",
      });
    }

    /* ✅ GET FACULTY FROM MAPPING */
    ...
    if (!facultyResult.rows.length) {
      return res.status(400).json({
        message: "Faculty not mapped to this user",
      });
    }
    ...
    for (let record of attendance) {
      ...
      await db.query(
        ...
        [
          record.student_id,
          class_id || null, // must be UUID or NULL
          subject_id,
          faculty_id,
          date,
          record.status,
