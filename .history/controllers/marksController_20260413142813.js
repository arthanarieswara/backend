...
const getSubjectMarks = async (req, res) => {
  const { semester, department_id, section, subject_id, test_type, test_no } =
    req.query;

  try {
    if (!semester || !department_id || !subject_id || !test_type) {
      return res.status(400).json({
        message: "semester, department_id, subject_id, test_type required",
      });
    }
    ...
    let query = `
      ...
      /* 🔥 LEFT JOIN (so all students appear) */
      LEFT JOIN marks m 
        ON m.student_id = s.id
        AND m.subject_id = $1
        AND m.test_type = $2
    ...
    let params = [parseInt(subject_id), test_type];
    ...
    query += `
      WHERE s.semester = $${params.length + 1}
      AND s.department_id = $${params.length + 2}
    `;
